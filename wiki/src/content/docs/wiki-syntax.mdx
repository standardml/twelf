...
into the editor when you click on the hat, and to inform the configuration sent to
...
of Twelf sections, so these two purposes can be individually modified.
...
This allows the configured MDX files to be much smaller, but can still lead to large
HTML assets, as the HTML assets will include the context in the "hat" links. To deal
with that problem, it's possible to overload the `UrlAction` (as defined by the
[`twelf-wasm` package](https://www.npmjs.com/package/twelf-wasm)) by defining a
hatAction parameter, which must be valid JSON.

In general, this can be used to confusing effect: when we render this code...
...
...clicking hat will open up a file unrelated to the file we expected, as you can
...
Twelf code loaded by the editor can be provided outside of the URL itself.
 ...
 * `<Keyword>` components appear at the bottom of pages that describe Twelf
   ...
   example, which uses `<Keyword name="covers"/> 

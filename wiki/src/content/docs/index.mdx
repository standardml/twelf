...
- The  declaration specifies the mode of a type family&mdash;in this case, that `N1` and `N2` are universally quantified (`+`) and that `N3` is existentially quantified (`-`). We will sometimes refer to the universally-quantified types as ''inputs'' and the existentially-quantified types as ''outputs''.

...
struct
  ...
  (* Invariants:
   ...
   - if modLookup m = ViewDec _, then symLookup(m,l) = SymCSInst _
   ...
   Here n = 0 iff a declaration is not generated by the module system.
  ...
  *)
  ...
  (* maps mids m to tuple of
     - module declarations (modLookup m)
     - size (-1 if the module is still open) (modSize m)
     - the value of ! scope immediately before the start of the module (modParent m)
     - list of includes (modInclLookup m)
  *)
  (* A signature M with modLookup(M) contains a symbol newcid(m,l) iff one of the following holds
     - m = M

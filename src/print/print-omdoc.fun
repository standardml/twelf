...
local
  ...
  fun replace c = if (Char.isAlphaNum c) orelse (Char.contains ":_-." c) then
 	(String.str c)
  	...
  	"_"
  fun Name (cid) = let
  	val name = String.translate replace (I.conDecName(I.sgnLookup cid))
  ...
  *)
  fun fmtExpW (G, (I.Uni(L), s), _) = sexp [fmtUni L]
    | fmtExpW (G, (I.Pi((D as I.Dec(_,V1),P),V2), s), imp) =
	 ...
	 of I.Maybe => let
			 val (D' as I.Dec (SOME(n), V1')) = Names.decLUName (G, D) (* could sometimes be EName *)
			 val G' = I.Decl (G, D')
			 val _ = ind(2)  (* temporary indentation *)
			 val fmtType = fmtExp (G, (V1', s), 0)
			 val _ = unind(2)
			 val pi = if (imp > 0) then "implicit_Pi" else "Pi"
			 val name = VarName(I.ctxLength G',n)
		       in
	"<om:OMBIND>" ^ nl_ind() ^ "<om:OMS cd=\"twelf\" name=\"" ^ pi ^ "\"/>" ^ nl() ^ "<om:OMBVAR><om:OMATTR>" ^
	"<om:OMATP><om:OMS cd=\"omdoc\" name=\"notation\"/><om:OMFOREIGN encoding=\"application/omdoc+xml\">" ^ nl_ind() ^
	"<presentation for=\"#" ^ name ^ "\"><use format=\"twelf\">" ^ n ^ "</use></presentation>" ^
	"</om:OMFOREIGN></om:OMATP>" ^ "<om:OMATP>" ^ nl() ^
	"<om:OMS cd=\"twelf\" name=\"oftype\"/>" ^ nl() ^
	fmtType ^ nl() ^ "</om:OMATP>" ^ nl() ^
	"<om:OMV name=\"" ^ name ^ "\"/>" ^ nl_unind() ^ "</om:OMATTR></om:OMBVAR>" ^ nl() ^
	fmtExp (G', (V2, I.dot1 s), Int.max(0,imp - 1)) ^ nl_unind() ^ "</om:OMBIND>"
		       end
	  | I.No => let
		       val G' = I.Decl (G, D)
		    in
    ...
    | fmtExpW (G, (I.Root (H, S), s), imp) = let
	val l = spineLength(S)
	val out = ref ""
	val _ = if (l = 0) then
		(* no arguments *)
    ...
    | fmtExpW (G, (I.Lam(D, U), s), imp) = 
      let
	val (D' as I.Dec (SOME(n), V)) = Names.decLUName (G, D)
	val G' = I.Decl (G, D')
	val _ = ind(2)  (* temporary indentation *)
	...
	val name = VarName(I.ctxLength G',n)
      in
	"<om:OMBIND>" ^ nl_ind() ^ "<om:OMS cd=\"twelf\" name=\"" ^ lam ^ "\"/>" ^ nl() ^ "<om:OMBVAR><om:OMATTR>" ^
	"<om:OMATP><om:OMS cd=\"omdoc\" name=\"notation\"/><om:OMFOREIGN encoding=\"application/omdoc+xml\">" ^ nl_ind() ^
	"<presentation for=\"#" ^ name ^ "\"><use format=\"twelf\">" ^ n ^ "</use></presentation>" ^
	"</om:OMFOREIGN></om:OMATP>" ^ "<om:OMATP>" ^ nl() ^
	"<om:OMS cd=\"twelf\" name=\"oftype\"/>" ^ nl() ^
	fmtType ^ nl() ^ "</om:OMATP>" ^ nl() ^
	"<om:OMV name=\"" ^ name ^ "\"/>" ^ nl_unind() ^ "</om:OMATTR></om:OMBVAR>" ^ nl() ^
	fmtExp (G', (V, I.dot1 s), Int.max(0,imp - 1)) ^ nl_unind() ^ "</om:OMBIND>"
      end
  ...
  *)
  ...
  (* top-level OMDoc output, used in fmtConDec *)
  ...
  and fmtPresentation(cid) = let
	...
	val bracString = "bracket-style=\"lisp\" lbrack=$brackOpen rbrack=$brackClose"
	val implicitString = "implicit=\"" ^ (Int.toString imp) ^ "\""
  in
  	"<presentation for=\"#" ^ (Name cid) ^ "\">" ^ nl_ind() ^
	"<use format=\"twelf\" " ^ fixString ^ " " ^ precString ^ " " ^ bracString ^ " " ^
	implicitString ^ ">" ^ (escape (I.conDecName(I.sgnLookup cid))) ^ "</use>" ^ nl_unind() ^ 
	"</presentation>"
  end
  ...
  and fmtFixity(cid) = let
	...
	"<data format=\"twelf\"><![CDATA[" ^ (Names.Fixity.toString fixity) ^ ".]]></data>" ^ nl_unind() ^
	"</private>"
      end
...
(*  fun formatSpine (G, S) = sexp (fmtSpine (G, (S, I.id))) *)
  ...
  fun printSgn () = (
      ind_reset();
      IntSyn.sgnApp (fn (cid) => (print ((formatConDec (cid, IntSyn.sgnLookup cid)));
				  print "\n")))
...
"version=\"1.2\">\n\n"
	...
	val _ = IntSyn.sgnApp (fn (cid) =>
		(TextIO.output (file, (formatConDec (cid, IntSyn.sgnLookup cid)) ^ 

  ...
  *)
  fun fmtExpW (G, (I.Uni(L), s), _) = sexp [fmtUni L]
	...
	sexp [Str ("<OMBIND> <OMS cd =\"LF\" name=\"" ^ pi ^ "\"/>"),
	      ...
	      F.Break, fmtExp (G', (V2, I.dot1 s), imp - 1), Str "</OMBIND>"]
		       end
	  | I.No => let
		       val G' = I.Decl (G, D)
		    in
		      sexp [Str "<OMA>", F.Break, Str "<OMS cd=\"LF\" name=\"arrow\"/>",
			    fmtExp (G, (V1, s), 0),
			    F.Break, fmtExp (G', (V2, I.dot1 s), 0),
			    Str "</OMA>"]
		    end)
    | fmtExpW (G, (I.Root (H, S), s), _) =
      (case (fmtSpine (G, (S, s)))
	 of NONE =>  fmtCon (G, H)
	...
	val lam = if (imp > 0) then "implicit_lambda" else "lambda"
      in
	sexp [Str ("<OMBIND> <OMS cd =\"LF\" name=\"" ^ lam ^ "\"/>"),
	      F.Break, Str "<OMBVAR>", F.Break, Str "<OMATTR>", F.Break,
	      Str "<OMATP>", F.Break,
	      Str "<OMS cd=\"LF\" name=\"typeof\"/>", F.Break,
	      fmtType, F.Break, Str "</OMATP>", F.Break, 
	      Str "<OMV name=",  Name (x),  Str "/></OMATTR></OMBVAR>",
	      F.Break, fmtExp (G', (U, I.dot1 s), imp - 1), Str "</OMBIND>"]
      end
  ...
  *)
  and fmtSpine (G, (I.Nil, _)) = NONE 
    ...
    | fmtSpine (G, (I.App(U, S), s)) =
      (case (fmtSpine (G, (S, s))) 
	 of NONE => SOME [fmtExp (G, (U, s), 0)]
  	  | SOME fmts => SOME ([fmtExp (G, (U, s), 0), F.Break] @ fmts))



  fun fmtExpTop (G, (U, s), imp) = sexp [Str "<OMOBJ>", fmtExp (G, (U, s), imp), Str "</OMOBJ>"]
